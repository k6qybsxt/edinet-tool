...
def _detail_rows(rows):
    return [row for row in rows if row.row_kind in {ROW_KIND_DETAIL, ROW_KIND_CHANGE_RATE}]


def _single_header_ending_with(headers: dict[str, int], suffix: str) -> str:
    matches = [header for header in headers if isinstance(header, str) and header.endswith(suffix)]
    if len(matches) != 1:
        raise AssertionError(f"expected one header ending with {suffix!r}, got {matches!r}")
    return matches[0]
...
class MetricExcelExportServiceTest(unittest.TestCase):
    ...
    def test_write_metric_excel_uses_requested_base_column_layout(self) -> None:
        output_path = self.tmp_path / "column_layout.xlsx"
        rows = [
            MetricExcelRow(
                sheet_name=GENERAL_SHEET,
                security_code="1111",
                ...
                industry_33="\u5316\u5b66",
                market="Prime",
                period_scope="quarter_standalone:1Q",
                current_period_end="2025-06-30",
                metric_base="NetSales",
                metric_label="1Q \u58f2\u4e0a\u9ad8 \u5358\u72ec",
                periods_by_offset={0: "1Q 2025-06"},
    ...
    def test_segment_rows_merge_sony_renamed_entertainment_segment(self) -> None:
        ...
        self.assertEqual(segment_rows[0].periods_by_offset[1], "4Q 2023-03")
        self.assertEqual(segment_rows[0].periods_by_offset[2], "4Q 2022-03")
    ...
    def test_build_rows_uses_fixed_metric_order(self) -> None:
        ...
        self.assertEqual(
            ...
            ["OutstandingShares", "NetAssets", "NetSales", "CostOfSales", "EPS"],
        )
    ...
    def test_build_rows_keeps_043000_half_filings_out_of_annual_offsets(self) -> None:
        ...
        self.assertEqual(
            ...
            ["4Q 2024-03", "4Q 2025-03", "4Q 2026-03"],
        )
        self.assertEqual(
    ...
    def test_half_rows_suppress_disabled_metrics(self) -> None:
        ...
        self.conn.executemany(
            ...
            [
                ...
                ("AverageLengthOfServiceCurrent", 12.3),
        ...
        self.conn.executemany(
            ...
            [
                ...
                ("OutstandingSharesGrowthRateCurrent", "OutstandingSharesGrowthRate"),
            ],
        )
        self.conn.commit()
        condition = MetricExcelCondition(
    ...
    def test_build_rows_uses_default_excel_metric_order_and_renamed_labels(self) -> None:
        condition = MetricExcelCondition(
            security_codes=["1111"],
            metric_labels=[
                "OutstandingShares",
        ...
        self.assertTrue(all(row.periods_by_offset[1] == "4Q 2026-03" for row in detail_rows))
    ...
    def test_cash_balance_metric_label_is_registered(self) -> None:
        ...
        self.assertEqual(detail_rows[0].metric_label, "4Q \u671f\u672b\u6b8b\u9ad8")
    ...
    def test_eps_growth_metric_label_uses_prior_period_suffix(self) -> None:
        ...
        self.assertEqual(detail_rows[0].row_kind, ROW_KIND_CHANGE_RATE)
    ...
    def test_growth_metric_labels_use_prior_period_suffix_for_annual_and_half(self) -> None:
        ...
        self.assertEqual(errors, [])
        detail_by_key = {
            (row.period_scope, row.metric_base): row
            for row in _detail_rows(rows)
        }
        ...
        for base, label in expected_base_labels.items():
            self.assertEqual(detail_by_key[("annual", base)].metric_label, f"4Q {label}")
            self.assertEqual(detail_by_key[("annual", base)].row_kind, ROW_KIND_CHANGE_RATE)
            self.assertEqual(detail_by_key[("quarter:2Q", base)].metric_label, f"2Q {label}")
            self.assertEqual(detail_by_key[("quarter:2Q", base)].row_kind, ROW_KIND_CHANGE_RATE)
    ...
    def test_export_metric_excel_writes_percent_format(self) -> None:
        ...
        ws = workbook[GENERAL_SHEET]
        headers = {cell.value: idx for idx, cell in enumerate(ws[1], start=1)}
        period_header = _single_header_ending_with(headers, "_\u671f\u9593")
        value_header = _single_header_ending_with(headers, "_\u6570\u5024")
        unit_header = _single_header_ending_with(headers, "_\u5358\u4f4d")
        ratio_header = _single_header_ending_with(headers, "_\u6bd4\u7387")
        self.assertEqual(ws.cell(2, headers["\u5e02\u5834\u533a\u5206"]).value, "Prime")
        self.assertEqual(ws.cell(2, headers["\u6c7a\u7b97\u7a2e\u5225"]).value, "4Q")
        self.assertEqual(ws.cell(2, headers["\u5024\u7a2e\u5225"]).value, VALUE_KIND_BASE)
        self.assertEqual(ws.cell(2, headers["\u6307\u6a19"]).value, "4Q ├売上原価")
        self.assertEqual(ws.cell(2, headers[period_header]).value, "4Q 2026-03")
        self.assertEqual(ws.cell(2, headers[value_header]).value, 60.0)
        self.assertEqual(ws.cell(2, headers[unit_header]).value, "百万円")
        ratio_cell = ws.cell(2, headers[ratio_header])
        self.assertEqual(ratio_cell.value, 0.6)
        self.assertEqual(ratio_cell.number_format, "0.0%")
    ...
    def test_export_metric_excel_formats_growth_rates_as_percent(self) -> None:
        ...
        value_header = _single_header_ending_with(headers, "_\u6570\u5024")
        unit_header = _single_header_ending_with(headers, "_\u5358\u4f4d")
        value_cell = ws.cell(2, headers[value_header])
        self.assertEqual(ws.cell(2, headers["\u6307\u6a19"]).value, "4Q 売上高増収率(前期比)")
        self.assertEqual(ws.cell(2, headers[period_header]).value, "4Q 2026-03")
        self.assertEqual(value_cell.value, 1.25)
        self.assertEqual(ws.cell(2, headers[unit_header]).value, "%")
        self.assertEqual(value_cell.number_format, "0.0%")
    ...
    def test_growth_rows_use_change_rate_row_kind_but_stats_keep_stat_kind(self) -> None:
        self.conn.executemany(
            """
            INSERT INTO derived_metrics (
                doc_id, edinet_code, security_code, metric_key, metric_base, metric_group,
                ...
                formula_name, source_detail_json, rule_version, created_at, updated_at
                      ...
                      'NetSalesGrowthRate', 'growth', 2025, ?, 'annual',
                      0, 'consolidated', 'Japan GAAP', 'ratio', ?, 'ratio',
                      'ok', 'test', '{}', 'v1', '2026-04-24', '2026-04-24')
            """,
            [
                ("E00001_0", "E00001", "11110", "2026-03-31", 1.25),
                ("E00002_0", "E00002", "22220", "2026-03-31", 0.75),
            ],
        )
        self.conn.commit()
        condition = MetricExcelCondition(
        ...
        self.assertEqual(
            ...
            [ROW_KIND_CHANGE_RATE, ROW_KIND_CHANGE_RATE, ROW_KIND_AVERAGE, ROW_KIND_MEDIAN],
        )
        average_row = next(row for row in rows if row.row_kind == ROW_KIND_AVERAGE)
        median_row = next(row for row in rows if row.row_kind == ROW_KIND_MEDIAN)
    ...
    def test_export_metric_excel_writes_row_kind_rank_without_vertical_data(self) -> None:
        ...
        ws = workbook[GENERAL_SHEET]
        headers = {cell.value: idx for idx, cell in enumerate(ws[1], start=1)}
        period_col = headers[_single_header_ending_with(headers, "_\u671f\u9593")]
        rank_col = headers[_single_header_ending_with(headers, "_\u9806\u4f4d")]
        ...
        self.assertEqual(ws.cell(1, period_col).fill.fgColor.rgb, "00EAF4FF")
        self.assertEqual(ws.cell(2, period_col).border.left.style, "thin")
        self.assertNotIn("\u30c7\u30fc\u30bf\u7528_\u7e26", workbook.sheetnames)
    ...
    def test_export_metric_excel_formats_per_share_and_theoretical_values(self) -> None:
        ...
        headers = {cell.value: idx for idx, cell in enumerate(ws[1], start=1)}
        formats_by_label = {
            ws.cell(row=row_index, column=headers["\u6307\u6a19"]).value: ws.cell(
                row=row_index,
                column=headers[_single_header_ending_with(headers, "_\u6570\u5024")],
            ).number_format
    ...
    def test_market_derived_metrics_are_loaded_with_date_point_period(self) -> None:
        ...
        self.assertEqual(by_base["StockPrice"].periods_by_offset[1], "4Q 2026-03-31\u6642\u70b9")
    ...
    def test_jquants_quarter_rows_use_forecast_progress_ratio(self) -> None:
        ...
        ws = workbook[GENERAL_SHEET]
        headers = {cell.value: idx for idx, cell in enumerate(ws[1], start=1)}
        ratio_cell = ws.cell(2, headers[_single_header_ending_with(headers, "_\u6bd4\u7387")])
        self.assertEqual(ratio_cell.value, 0.25)
    ...
    def test_ordinary_income_growth_label_follows_ifrs_profit_before_tax_display(self) -> None:
        self.conn.execute(
            "UPDATE filings SET accounting_standard = 'IFRS' WHERE edinet_code = 'E00001'"
        )
        self.conn.executemany(
            """
            INSERT INTO jquants_financial_metrics (
                ...
                source_field, source_detail_json, rule_version, created_at, updated_at
                      ...
                      'quarter', 'actual:1Q', '1Q', NULL, ?,
                      ?, ?, ?, '15:00', 'ProfitBeforeTaxCurrent',
                      'ProfitBeforeTax', 'profitability', ?, 'yen', 'ok',
        ...
        condition = MetricExcelCondition(
            security_codes=["1111"],
            metric_labels=["OrdinaryIncomeGrowthRate"],
            period_scopes=["quarter"],
            period_offsets=[0],
        )
        ...
        self.assertNotIn("jquants_metrics_not_found", warnings)
        row = next(row for row in _detail_rows(rows) if row.period_scope == "quarter:1Q")
        self.assertEqual(row.metric_base, "OrdinaryIncomeGrowthRate")
        self.assertEqual(
            row.metric_label,
            "1Q \u7a0e\u5f15\u524d\u5229\u76ca\u5897\u76ca\u7387(\u524d\u671f\u6bd4)",
        )
    ...
    def test_jquants_quarter_cash_balance_growth_uses_saved_derived_rows(self) -> None:
        self.conn.executemany(
            """
            INSERT INTO jquants_financial_metrics (
                disclosure_number, local_code, security_code, edinet_code, metric_kind,
                ...
                source_field, source_detail_json, rule_version, created_at, updated_at
                      ...
                      'quarter', ?, ?, NULL, ?,
                      ?, ?, ?, '15:00', 'CashBalanceGrowthRateCurrent',
                      'CashBalanceGrowthRate', 'growth', ?, 'ratio', 'ok',
                      'calculated:CashAndCashEquivalents/prior_year_same_quarter',
                      '{}', 'v1',
                      '2026-05-06', '2026-05-06')
            """,
            [
                ("q1_growth", "actual:1Q", "1Q", 2026, "2025-04-01", "2025-06-30", "2025-08-01", 2.0),
                ("q2_growth", "actual:2Q", "2Q", 2026, "2025-04-01", "2025-09-30", "2025-11-01", 1.5),
                ("q3_growth", "actual:3Q", "3Q", 2026, "2025-04-01", "2025-12-31", "2026-02-01", 2.0),
            ],
        )
        self.conn.commit()
        condition = MetricExcelCondition(
            security_codes=["1111"],
            metric_labels=["CashBalanceGrowthRate"],
            period_scopes=["quarter"],
            period_offsets=[0],
        )
        ...
        by_scope = {
            ...
            for row in _detail_rows(rows)
            if row.metric_base == "CashBalanceGrowthRate"
        }
    ...
    def test_annual_cash_balance_growth_is_shown_as_4q(self) -> None:
        self.conn.execute(
            """
            INSERT INTO derived_metrics (
                doc_id, edinet_code, security_code, metric_key, metric_base, metric_group,
                ...
                accounting_standard, document_display_unit, value_num, value_unit, calc_status,
                formula_name, source_detail_json, rule_version, created_at, updated_at
            ) VALUES ('E00001_0', 'E00001', '11110', 'CashBalanceGrowthRateCurrent',
                      'CashBalanceGrowthRate', 'growth', 2025, '2026-03-31',
                      'annual', 0, 'consolidated', 'Japan GAAP', '',
                      1.25, 'ratio', 'ok', 'test', '{}', 'v1',
                      '2026-04-24', '2026-04-24')
            """
        )
        self.conn.commit()
        ...
        rows, errors, _warnings, _preview, target = build_metric_excel_rows(self.conn, condition)
        ...
        row = next(
            row
            for row in _detail_rows(rows)
            if row.period_scope == "annual" and row.metric_base == "CashBalanceGrowthRate"
        )
        self.assertEqual(row.metric_label, "4Q \u671f\u672b\u6b8b\u9ad8\u5897\u52a0\u7387(\u524d\u671f\u6bd4)")
        self.assertAlmostEqual(row.values_by_offset[1], 1.25)
        self.assertEqual(row.units_by_offset[1], "%")
    ...
    def test_obsolete_quarter_standalone_fcf_growth_rows_are_suppressed(self) -> None:
        self.conn.execute(
            """
            ...
            VALUES ('1111', 'E00001', 2026, '2Q', '2025-09-30',
                    'FCFGrowthRateQuarterStandaloneCurrent', 'FCFGrowthRate', 'growth', 1.2, 'ratio',
        ...
        self.assertFalse(
            [
                ...
                if row.period_scope
                in {
                    "quarter_standalone:1Q",
                    "quarter_standalone:2Q",
                    "quarter_standalone:3Q",
                    "quarter_standalone:4Q",
                }
            ]
        )

    def test_quarter_standalone_pl_rows_use_quarter_labels_not_half_labels(self) -> None:
        self.conn.executemany(
            """
            INSERT INTO quarter_standalone_metrics (
                security_code, edinet_code, fiscal_year, quarter_type, period_end,
            ...
            """,
            [
                ("1Q", "2025-06-30", "NetSalesCurrent", "NetSales", "sales", 100_000_000.0, "yen"),
                ("2Q", "2025-09-30", "NetSalesCurrent", "NetSales", "sales", 110_000_000.0, "yen"),
                ("3Q", "2025-12-31", "NetSalesCurrent", "NetSales", "sales", 120_000_000.0, "yen"),
                ("4Q", "2026-03-31", "NetSalesCurrent", "NetSales", "sales", 130_000_000.0, "yen"),
                ("1~2Q", "2025-09-30", "NetSalesCurrent", "NetSales", "sales", 999_000_000.0, "yen"),
                ("3~4Q", "2026-03-31", "NetSalesCurrent", "NetSales", "sales", 998_000_000.0, "yen"),
                (
                    "1Q",
                    "2025-06-30",
                    "NetSalesGrowthRateQuarterStandaloneCurrent",
                    "NetSalesGrowthRate",
                    "growth",
                    1.1,
                    "ratio",
        ...
        condition = MetricExcelCondition(
            security_codes=["1111"],
            metric_labels=["NetSales", "NetSalesGrowthRate"],
            period_scopes=["quarter_standalone"],
            period_offsets=[0],
        )
        ...
        net_sales_by_scope = {
            row.period_scope: row
            for row in _detail_rows(rows)
            if row.metric_base == "NetSales"
        }
        growth_by_scope = {
            row.period_scope: row
            for row in _detail_rows(rows)
            if row.metric_base == "NetSalesGrowthRate"
        }
        self.assertEqual(set(net_sales_by_scope), expected_scopes)
        self.assertEqual(set(growth_by_scope), expected_scopes)
        self.assertEqual(net_sales_by_scope["quarter_standalone:2Q"].values_by_offset[0], 110.0)
        self.assertEqual(growth_by_scope["quarter_standalone:2Q"].values_by_offset[0], 1.2)
        self.assertEqual(
            [
                decision_label_for_excel(net_sales_by_scope[f"quarter_standalone:{quarter}"])
    ...
    def test_quarter_standalone_pl_4q_rows_are_loaded(self) -> None:
        self.conn.executemany(
            """
            INSERT INTO quarter_standalone_metrics (
                security_code, edinet_code, fiscal_year, quarter_type, period_end,
    ...
    def test_forecast_revision_values_use_font_color_only(self) -> None:
        ...
        value_cells_by_label = {
            ...
            for row_index in range(2, ws.max_row + 1)
            if ws.cell(row=row_index, column=headers["\u5024\u7a2e\u5225"]).value == VALUE_KIND_BASE
        }
