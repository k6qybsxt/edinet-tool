...
from edinet_pipeline.services.zip_loader import extract_selected_xbrl, list_xbrl_members_in_zip
...
def _build_zip_item(zip_path: str, member_name: str, xbrl_name: str) -> dict | None:
...
def _pick_company_name(items: list[dict]) -> str | None:
    for item in items:
...
def _sort_job_items(items: list[dict]) -> list[dict]:
    sorted_items = list(items)
    sorted_items.sort(
        key=lambda x: (x.get("period_end", ""), x.get("xbrl_name", "")),
        reverse=True,
    )
    return sorted_items


def _select_company_files(items: list[dict]) -> tuple[bool, list[dict]] | None:
    half_items = _sort_job_items([x for x in items if x.get("doc_type") == "half"])
    annual_items = _sort_job_items([x for x in items if x.get("doc_type") == "annual"])
    ...
    if has_half:
        if len(annual_items) < 2:
            return None
        return True, [half_items[0], annual_items[0], annual_items[1]]

    if len(annual_items) < 3:
        return None

    return False, [annual_items[0], annual_items[1], annual_items[2]]


def _extract_selected_paths(selected: list[dict], extract_root: str | None = None) -> list[str]:
    extract_base = (
        Path(extract_root)
        if extract_root
        else (Path(selected[0]["zip_path"]).parent.parent / "_zip_extracted")
    )
...
def build_company_job(company_code: str, items: list[dict], extract_root: str | None = None) -> dict | None:
    ...
    selected_result = _select_company_files(items)
    if selected_result is None:
        return None

    has_half, selected = selected_result
    extracted_paths = _extract_selected_paths(selected, extract_root=extract_root)
    company_name = _pick_company_name(items)
...
def build_all_company_jobs(zip_dir: Path, extract_root: str | None = None) -> list[dict]:
    ...
    for company_code in sorted(grouped.keys()):
        job = build_company_job(
            company_code=company_code,
            items=grouped[company_code],
...
def finalize_company_jobs(jobs, max_companies):
    selected_jobs = list(jobs[:max_companies])
